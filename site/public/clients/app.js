...
var App = new Vue({
    ...
    methods: {
        ...
        getDb: function (cb) {
            ...
            for (var i in db.products) {
                var product = db.products[i];

                if (typeof categories[`cat_${product.category_id}`] === 'undefined') {
                    categories[`cat_${product.category_id}`] = {
                ...
                categories[`cat_${product.category_id}`].products.push({
            ...
            App.categories = Object.values(categories);
            App.whatsapp.phone = db.whatsapp;
            App.order_host = db.order_host;
...
*TOTAL:* R$ R$ ${number_format(total, 2, ',', '.')}`;
        ...
        createOrder: function () {
            // alert('createOrder');
            // App.goToPage('/whatsapp-link')


            axios.post(App.order_host)
                .then(function (ajaxResponse) {
                    console.log('ajaxResponse', ajaxResponse);
                    // App.categories = db.categories;
                    // App.whatsapp.phone = db.whatsapp.phone;
                    // cb();
                }).catch(function (ajaxResponse) {
                    console.log('ajaxResponse.response.data', ajaxResponse.response.data);
                });
        },
    },
});
